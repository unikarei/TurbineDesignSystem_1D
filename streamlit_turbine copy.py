...
def design_iteration_with_solidity_history(
    ...
    for i in range(1, max_iter + 1):
        ...
        V2 = math.sqrt(2 * delta_h_r)  # rotor側が動翼落差 → stator出口速度はこれを使う場合の解釈もあるが落差配分に合わせる

        # ここでは静翼出口速度は rotor/ statorの落差から同様に V2_stator = sqrt(2*delta_h_s)
        V2_s = math.sqrt(2 * delta_h_s)
...
with st.sidebar:
    ...
    T0 = st.number_input("入口全温 T0 [K]", value=1500.0, format="%.1f")
    P0 = st.number_input("入口全圧 P0 [Pa]", value=3e6, format="%.3e")
    P_out = st.number_input("出口静圧 P_out [Pa]", value=1.5e6, format="%.3e")
    mdot = st.number_input("質量流量 ṁ [kg/s]", value=25.0, format="%.3f")
...
st.markdown("### 収束過程（反動度・翼長の変化）")
st.line_chart(history_df[["R", "h_s", "h_r"]].rename(columns={"h_s": "h_s (静翼)", "h_r": "h_r (動翼)"}))

st.markdown("### 収束誤差（対数目盛）")
err_plot = history_df[["err_R", "err_hs", "err_hr"]].copy()
err_plot.columns = ["反動度誤差", "静翼高さ誤差", "動翼高さ誤差"]
st.line_chart(err_plot.apply(lambda x: x + 1e-16))  # ゼロ防止

st.markdown("### 反復履歴（上位10行）")
